...
interface Ctx {
  env: NoxDatabaseEnv & { ENCRYPTION_KEY?: string; TASK_QUEUE?: Queue };
...
export async function onRequestPatch(context: Ctx): Promise<Response> {
  ...
  const existing = await db.prepare(
    "SELECT id, slack_channel_id, widget_config FROM spot_sites WHERE id = ? AND org_id = ?",
  ...
  if (input.slackChannelId) {
    const slack = await resolveSlackInstall({ ...context.env, DB: db }, orgId);
  ...
  await db.prepare(
    `UPDATE spot_sites SET slack_channel_id = ?, widget_config = ?,
