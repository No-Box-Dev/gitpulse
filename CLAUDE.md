...
Admins complete Slack OAuth once per organization, then configure routing under `settings.slack`: `fallbackChannelId`, `noxAlertChannelId`, `unticketChannelId`, `postsChannelId`, and `releaseNotesChannelId`. NoxFeed's optional `noxFeedProjectId` scopes both Slack streams to one project; missing/empty means all projects and never filters the in-app feed. NoxSpot keeps an optional per-site override in `spot_sites.slack_channel_id`; without it, the organization fallback is used. The precedence is always service/site-specific first, fallback second. The bot token is stored encrypted per-org in `slack_settings`; product configuration stores only public channel IDs. The briefly-used combined `noxFeedChannelId` is adopted for both NoxFeed streams on read and removed when the split routes are saved.
...
- Endpoints: `POST /api/slack/oauth/start` (admin, returns Slack authorize URL + sets CSRF cookie; optional body `{ team }` pins the authorize page to a workspace — a team ID pins that workspace, `""`/`null` opens Slack's picker, omitted pins the org's currently connected workspace), `GET /api/slack/oauth/callback` (middleware-bypassed; verifies cookie, exchanges code, persists install, redirects to `/?tab=integrations&slack=ok`), `GET /api/slack/status`, `GET /api/slack/channels` (admin), `POST /api/slack/disconnect` (admin), `POST /api/slack/test` (admin; posts a sample to a given channel).
- Routing: NoxFeed narratives use `postsChannelId` and release summaries use `releaseNotesChannelId`; Unticket issue opened/closed/reopened activity uses `unticketChannelId`; automatic NoxSpot errors and their GitHub issue closures use `noxAlertChannelId`; ordinary NoxSpot captures use their site channel. Every path falls back to `fallbackChannelId`. Slack messages are staged in `delivery_outbox`; delivery failures do not block the in-app or GitHub source of truth.
...
The organization onboarding surface shows the required GitHub connection, optional shared Slack connection, readiness cards, and service-specific channel routing for NoxFeed, NoxSpot, NoxAlert, and Unticket. GitHub unlocks NoxFeed and NoxSpot; Slack delivery makes NoxAlert ready. Connection and feature states are derived server-side by `/api/integrations/status`, which returns operational metadata and public channel IDs but never credentials.
