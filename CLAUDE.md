...
Admins complete Slack OAuth once per organization, then configure routing under `settings.slack`: `fallbackChannelId`, `noxAlertChannelId`, `unticketChannelId`, `postsChannelId`, and `releaseNotesChannelId`. NoxSpot keeps an optional per-site override in `spot_sites.slack_channel_id`; without it, the organization fallback is used. The precedence is always service/site-specific first, fallback second. The bot token is stored encrypted per-org in `slack_settings`; product configuration stores only public channel IDs. The briefly-used combined `noxFeedChannelId` is adopted for both NoxFeed streams on read and removed when the split routes are saved.
