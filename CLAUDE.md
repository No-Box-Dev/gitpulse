...
Admins complete Slack OAuth once per organization, then configure routing under `settings.slack`: `fallbackChannelId`, `noxAlertChannelId`, `noxTicketChannelId`, `postsChannelId`, and `releaseNotesChannelId`. NoxSpot keeps an optional per-site override in `spot_sites.slack_channel_id`; without it, the organization fallback is used. The precedence is always service/site-specific first, fallback second. The bot token is stored encrypted per-org in `slack_settings`; product configuration stores only public channel IDs. The briefly-used combined `noxFeedChannelId` is adopted for both NoxFeed streams on read and removed when the split routes are saved.
...
- Endpoints: `POST /api/slack/oauth/start` (admin, returns Slack authorize URL + sets CSRF cookie; optional `{ team }` pins a workspace, while `null` opens Slack's workspace picker), `GET /api/slack/oauth/callback` (middleware-bypassed; verifies cookie, exchanges code, persists install, redirects to `/?tab=integrations&slack=ok`), `GET /api/slack/status`, `GET /api/slack/channels` (admin), `POST /api/slack/disconnect` (admin), `POST /api/slack/test` (admin; posts a sample to a given channel).
- Routing: NoxFeed narratives use `postsChannelId` and release summaries use `releaseNotesChannelId`; NoxTicket issue opened/closed/reopened activity uses `noxTicketChannelId`; automatic NoxSpot errors and their GitHub issue closures use `noxAlertChannelId`; ordinary NoxSpot captures use their site channel. Every path falls back to `fallbackChannelId`. Slack messages are staged in `delivery_outbox`; delivery failures do not block the in-app or GitHub source of truth.
