...
function makeCtx({ row = null, body, isAdmin = true, orgId = 7, key = "managed-key" } = {}) {
  ...
  const DB = {
    prepare(sql) {
      return { sql, args: [], bind(...args) { this.args = args; return this; }, async first() { return row; } };
    },
    async batch(statements) { calls.batches.push(statements); return statements.map(() => ({ success: true })); },
  };
  return {
    ctx: {
...
describe("managed AI settings", () => {
  it("returns the managed default without exposing a key", async () => {
    ...
    expect(await response.json()).toEqual({
      mode: "managed",
      managed: { provider: "anthropic", model: "claude-haiku-4-5-20251001", available: true },
    });
  });
  ...
  it("requires organization admin access", async () => {
