...
function context({ count = 1, projectFound = true } = {}) {
        ...
        bind: (...binds: unknown[]) => {
          ...
          return {
            first: async () => {
              ...
              if (sql.includes("FROM projects")) return projectFound ? { name: "Web" } : null;
              if (sql.includes("FROM slack_connections")) return { id: "conn-1" };
              return null;
            },
  ...
  return {
    calls,
    value: {
      request: new Request("https://app.unticket.ai/api/slack/connections/conn-1", {
        method: "PATCH",
...
describe("PATCH /api/slack/connections/:id", () => {
  it("assigns an owned active project", async () => {
    ...
    expect(ctx.calls.some(({ sql, binds }) => sql.includes("UPDATE slack_connections")
      && binds[0] === "proj_acme_web")).toBe(true);
  });
  ...
  it("rejects a project from another organization", async () => {
    const response = await onRequestPatch(context({ projectFound: false }).value as never);
