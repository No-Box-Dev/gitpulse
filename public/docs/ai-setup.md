...
- NoxCue sources: `GET/POST /api/cues/sources`, `PUT /api/cues/sources/{sourceId}`, and `POST /api/cues/sources/{sourceId}/keys`. Configure each source’s Slack workspace/channel, IANA timezone, and local delivery time. A newly created server key is returned only once. Submit closed `user.registered` and `user.active` events to the stable same-origin gateway `POST /api/cues/public/v1/events`; NoxCue derives totals and rolling uniques available through `GET /api/cues/metrics?sourceId=...`. The ingest key belongs in `X-Nox-Ingest-Key`, not the Nox bearer-token headers. Reusing the same event identity is idempotent.
