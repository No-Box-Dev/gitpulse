...
export function PublicNoxCueDashboardPage() {
  ...
  return <main className="min-h-screen bg-[#101419] text-stone-100">
    ...
    {!source ? <div className="mx-auto max-w-6xl px-5 py-12 text-stone-400">No NoxCue environments are linked to this project.</div> : <Dashboard key={source.id} source={source} />}
...
function Dashboard({ source }: { source: DashboardSource }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-8 px-5 py-9 sm:px-8">
    ...
    <section><div className="mb-4"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-stone-500">Errors</p><h2 className="mt-1 font-display text-2xl">Recently observed</h2></div>{source.errors.length ? <div className="grid gap-3 sm:grid-cols-2">{source.errors.map((item, index) => {
      const itemKey = `${item.title}:${item.errorCode ?? ""}:${index}`;
      const expanded = expandedError === itemKey;
      return <article key={itemKey} className={`rounded-2xl border bg-[#181e26] transition ${expanded ? "border-blue-400/30 sm:col-span-2" : "border-white/10"}`}>
        <button type="button" aria-expanded={expanded} aria-controls={`error-log-${index}`} onClick={() => setExpandedError(expanded ? null : itemKey)} className="w-full p-4 text-left">
          <div className="flex items-start justify-between gap-3"><p className="font-medium text-stone-200">{item.title}</p><span className="shrink-0 rounded-full bg-white/5 px-2 py-1 text-xs text-stone-400">{item.occurrenceCount}×</span></div>
          <p className="mt-2 text-xs text-stone-500">{[item.component, item.errorCode].filter(Boolean).join(" · ") || "No error code"}</p>
          <div className="mt-3 flex items-center justify-between gap-3 text-[11px] text-stone-500"><span className="flex items-center gap-1.5"><Clock3 size={12} /> Last seen {formatDate(item.lastSeenAt)}</span><span className="flex items-center gap-1 text-blue-300">{expanded ? "Hide logs" : item.occurrences.length ? `View ${item.occurrences.length} log${item.occurrences.length === 1 ? "" : "s"}` : "View logs"}<ChevronDown size={13} className={`transition-transform ${expanded ? "rotate-180" : ""}`} /></span></div>
        </button>
        {expanded ? <div id={`error-log-${index}`} className="border-t border-white/10 px-4 pb-4 pt-3">
          {item.occurrences.length ? <div className="space-y-3">{item.occurrences.map((log, logIndex) => <div key={log.id} className="rounded-xl bg-black/20 p-4">
            <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-medium text-stone-300">Occurrence {item.occurrenceCount - logIndex}</p><p className="text-[11px] text-stone-500">{formatDate(log.occurredAt)}</p></div>
            {log.message ? <pre className="mt-3 whitespace-pre-wrap break-words font-sans text-xs leading-5 text-stone-300">{log.message}</pre> : <p className="mt-3 text-xs italic text-stone-500">No error message was supplied.</p>}
            <div className="mt-3 flex flex-wrap gap-2">{[log.component, log.errorCode, log.environment].filter(Boolean).map((value) => <span key={value} className="rounded-full bg-white/5 px-2 py-1 text-[10px] text-stone-400">{value}</span>)}{log.fatal ? <span className="rounded-full bg-red-400/10 px-2 py-1 text-[10px] text-red-300">Fatal</span> : null}{log.unhandled ? <span className="rounded-full bg-red-400/10 px-2 py-1 text-[10px] text-red-300">Unhandled</span> : null}</div>
            {log.url ? <a href={log.url} target="_blank" rel="noreferrer" className="mt-3 inline-flex max-w-full items-center gap-1 text-[11px] text-blue-300 hover:text-blue-200"><span className="truncate">{log.url}</span><ExternalLink size={11} className="shrink-0" /></a> : null}
          </div>)}</div> : <p className="text-xs text-stone-500">Detailed logs were not retained for this older error group.</p>}
          {item.occurrenceCount > item.occurrences.length ? <p className="mt-3 text-[11px] text-stone-500">Showing the latest {item.occurrences.length} retained occurrence{item.occurrences.length === 1 ? "" : "s"} of {item.occurrenceCount} total.</p> : null}
        </div> : null}
      </article>;
